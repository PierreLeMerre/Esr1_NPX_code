...
load('/Users/pierre/Documents/MATLAB/allenCCF/cmap_allen.mat')
...
glm = 1;
subset = 'tmu_OR'; %'tmu_OR', 'tmu_opto_neg' or just a cluster number;
...
% initialize
P = nan(3,f_stop-f_start,4,round(time_window/bin_sz));
...
for f = f_start : f_stop
    ...
    if task_mod
        ...
        if plotsubset==0
            ...
            %             load([Path2Ana 'Task_modulated_relative/Block' num2str(Block_nb) '/' id{1} '_' id{2} '_' subset '.mat'])
            %             tmu1 = tm_rel.unit;
            %             load([Path2Ana 'Task_modulated_relative/Block' num2str(Block_nb2) '/' id{1} '_' id{2} '_' subset '.mat'])
            %             tmu2 = tm_rel.unit;
            %             tmu = unique([tmu1' tmu2']);
        elseif plotsubset==1
            % Relative subset
            if relative == 1
                load([Path2Ana 'Task_modulated_relative/Block' num2str(Block_nb) '/' id{1} '_' id{2} '_' subset '.mat'])
                if strcmp(subset,'task_modulated_rel_opto')
                    tmu = tm_rel_opto.unit;
                elseif strcmp(subset,'task_modulated_rel_sound')
                    tmu = tm_rel_sound.unit;
                elseif strcmp(subset,'task_modulated_rel_airpuff')
                    tmu = tm_rel_airpuff.unit;
                elseif strcmp(subset,'task_modulated_rel_all')
                    load([Path2Ana 'Task_modulated_relative/Block' num2str(2) '/' id{1} '_' id{2} '_' subset '.mat'])
                    tmu1 = tm_rel.relative_cs;
                    tmu2 = tm_rel.relative_us;
                    load([Path2Ana 'Task_modulated_relative/Block' num2str(4) '/' id{1} '_' id{2} '_' subset '.mat'])
                    tmu3 = tm_rel.relative_cs;
                    tmu4 = tm_rel.relative_us;
                    tmu = unique([tmu1 tmu2 tmu3 tmu4]);
                end
                %Absolute subset
            elseif absolute==1
                load([Path2Ana 'Task_modulated_units/Block' num2str(Block_nb) '/' id{1} '_' id{2} '_' subset '.mat'])
                ...
                % Trace conditioning subset
            elseif conditioning ==1
                load([Path2Ana 'trace_conditioned_units/Block' num2str(Block_nb) '/' id{1} '_' id{2} '_' subset '.mat'])
                
                % Clustering subset
            elseif clustering ==1
                load([Path2Ana 'Clustering/U_ID.mat'])
                tmu = U_ID.unit(strcmp(U_ID.file,[id{1} '_' id{2}]) & U_ID.cluster==S);
                tmu = cell2mat(tmu);
                
                % GLM subset
            elseif glm ==1
                %                 load([Path2Ana 'Task_modulated_GLM/' id{1} '_' id{2} '_sound.mat'])
                %                 tmu1 = tmu;
                %                 load([Path2Ana 'Task_modulated_GLM/' id{1} '_' id{2} '_opto.mat'])
                %                 tmu2 = tmu;
                %                 load([Path2Ana 'Task_modulated_GLM/' id{1} '_' id{2} '_airpuff.mat'])
                %                 tmu3 = tmu;
                %                 load([Path2Ana 'Task_modulated_GLM/' id{1} '_' id{2} '_emg.mat'])
                %                 tmu4 = tmu;
                %                 tmu = [];
                %                 tmu = unique([tmu1.event tmu2.event tmu3.event tmu4.event]);
...
for f = f_start : f_stop
    ...
    if plotsubset==0
        load(['/Users/pierre/Documents/MATLAB/Code_10122021/neuropixelPFC/Matlab/analysis/Task_modulated_relative/Block2/' id{1} '_' id{2} '_task_modulated_rel_all.mat'])
        tm_rel_b2 = tm_rel;
        % sound 1
        bol_s1 = ((tm_rel_b2.relative_cs>1 & tm_rel_b2.relative_us>-0.5 & tm_rel_b2.relative_us<1) | (tm_rel_b2.relative_cs<-0.5 & tm_rel_b2.relative_us>-0.5 & tm_rel_b2.relative_us<1));
        % opto
        bol_o = ((tm_rel_b2.relative_us>1 & tm_rel_b2.relative_cs>-0.5 & tm_rel_b2.relative_cs<1) | (tm_rel_b2.relative_us<-0.5 & tm_rel_b2.relative_cs>-0.5 & tm_rel_b2.relative_cs<1));
        
        load(['/Users/pierre/Documents/MATLAB/Code_10122021/neuropixelPFC/Matlab/analysis/Task_modulated_relative/Block4/' id{1} '_' id{2} '_task_modulated_rel_all.mat'])
        tm_rel_b4 = tm_rel;
        % sound 2
        bol_s2 = ((tm_rel_b4.relative_cs2>1 & tm_rel_b4.relative_us2>-0.5 & tm_rel_b4.relative_us2<1) | (tm_rel_b4.relative_cs2<-0.5 & tm_rel_b4.relative_us2>-0.5 & tm_rel_b4.relative_us2<1));
        % airpuff
        bol_a = ((tm_rel_b4.relative_us2>1 & tm_rel_b4.relative_cs2>-0.5 & tm_rel_b4.relative_cs2<1) | (tm_rel_b4.relative_us2<-0.5 & tm_rel_b4.relative_cs2>-0.5 & tm_rel_b4.relative_cs2<1));
    ...
    for i = 1 : numel(unit_ids)
        if task_mod==1
            if ~isempty(tmu)
                for j = 1 : numel(tmu)
                    if i == tmu(j)
                        ...
                        if plotsubset==0
                            %relative modulation
                            for k = 1 : numel(tm_rel_b2.unit)
                                if i == tm_rel_b2.unit(k) && bol_s1(k)==1
                                    unit_modul(1,cnt) = 1;
                                end
                                if i == tm_rel_b2.unit(k) && bol_o(k)==1
                                    unit_modul(2,cnt) = 2;
                                end
                            end
                            for k = 1 : numel(tm_rel_b4.unit)
                                if i == tm_rel_b4.unit(k) && bol_s2(k)==1
                                    unit_modul(3,cnt) = 3;
                                end
                                if i == tm_rel_b4.unit(k) && bol_a(k)==1
                                    unit_modul(4,cnt) = 4;
                                end
                            end
                        elseif plotsubset==1
            ...
            if  isregion(i)==1 && isi_perc(i)<MIN_ISI && mean_session_fr(i)>MIN_FR
                ...
                % stereotaxic cordinates
                aap(cnt) = ap(main_ch(i)+1); % 0 indexed !!
                mml(cnt) = ml(main_ch(i)+1);
                ddv(cnt) = dv(main_ch(i)+1);
                reg{cnt} = ede_regions(main_ch(i)+1,:);
                cnt = cnt + 1;
...
if glm == 1
    ...
    U_ID.unit = U_ID.unit(Active_bins>200);
    U_ID.file = U_ID.file(Active_bins>200);
end
...
%text(1:length(explained(1:20)),explained(1:20),num2str(explained(1:20)),'vert','bottom','horiz','center');
xlim([1 20])
...
%     for i = 1:4
%         subplot(3,4,i+cnt)
%         plot(squeeze(nanmean(P(j,:,i,:),2)),'k')
%         title(['BLock ' num2str(i)])
%     end
%     cnt = cnt + i;
% end

%Make feature matrix as the dot product units by 10 first coefs to obtain a 10D space
%Feat = X_all*coef(:,1:10);
Feat = coef(:,1:20);
...
%         x2plot = X_all(T==outperm(i),:);
%         gen2plot = genotype_all(T==outperm(i))';
%         modul2plot = modul_all(:,T==outperm(i))';
%         modul2plot2 = modul_all2(:,T==outperm(i))';
%         modul_frac(i,:) = [sum(modul_all(1,T==outperm(i))==1) sum(modul_all(2,T==outperm(i))==2)...
%             sum(modul_all(3,T==outperm(i))==3) sum(modul_all(4,T==outperm(i))==4)];
%         comodul_frac(i,1) = sum(modul_all(2,T==outperm(i))==2 & modul_all(4,T==outperm(i))==4);
%         [~,order]=sort(gen2plot);
%         ConcX = [ConcX; x2plot(order,:)];
%         %imagesc(x2plot(order,:))
%         ylabel(['Cluster ' num2str(outperm(i))] )
%         %subplot(10,4,4+cnt)
...
%         colormap(inferno);
%         
%         sgtitle(['Cluster ' num2str(outperm(i))])
...
%         h = 0;
%         for i=1:numel(nb_of_units_in_clu)-1
%             h = h + nb_of_units_in_clu(outperm(i));
...
%     h = 0;
%     for i=1:numel(nb_of_units_in_clu)-1
%         h = h + nb_of_units_in_clu(outperm(numel(nb_of_units_in_clu)-i+1));
