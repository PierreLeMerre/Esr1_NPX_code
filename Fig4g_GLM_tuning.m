...
for f = f_start : f_stop
    ...
    n_predictors = 5;    
    X = [time_bin' sound_bin' emg_bin' opto_bin' air_bin']; %predictors n by p of p predictors %pupil_bin' emg_bin'
...
%          ax2 = subplot(1,5,2);
%         histogram(pupil_t,'BinLimits',[-20 20],'BinEdges',-50:1:50)
%         hold on
%         xlim([-50 50])
%          title('Pupil')
        
        ax3 = subplot(1,5,3);
        histogram(emg_t,'BinLimits',[-50 50],'BinEdges',-50:1:50)
        hold on
        xlim([-50 50])
        title('EMG')
        
        ax4 = subplot(1,5,4);
        histogram(opto_t,'BinLimits',[-20 20],'BinEdges',-20:1:20)
        hold on
        xlim([-20 20])
        title('Opto')
