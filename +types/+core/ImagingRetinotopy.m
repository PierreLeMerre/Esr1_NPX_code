...
properties
    ...
    focal_depth_image_format; % Format of image. Right now only 'raw' is supported.
    ...
    vasculature_image_format; % Format of image. Right now only 'raw' is supported.
end
...
methods
    ...
    function refs = export(obj, fid, fullpath, refs)
        ...
        if ~isempty(obj.axis_1_phase_map)
            ...
            error('Property `axis_1_phase_map` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.axis_1_phase_map_dimension) && ~isempty(obj.axis_1_phase_map)
            io.writeAttribute(fid, [fullpath '/axis_1_phase_map/dimension'], obj.axis_1_phase_map_dimension);
        elseif ~isempty(obj.axis_1_phase_map)
            error('Property `axis_1_phase_map_dimension` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.axis_1_phase_map_field_of_view) && ~isempty(obj.axis_1_phase_map)
            io.writeAttribute(fid, [fullpath '/axis_1_phase_map/field_of_view'], obj.axis_1_phase_map_field_of_view);
        elseif ~isempty(obj.axis_1_phase_map)
            error('Property `axis_1_phase_map_field_of_view` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.axis_1_phase_map)
            error('Property `axis_1_phase_map_unit` is required in `%s`.', fullpath);
        end
        ...
        if ~isempty(obj.axis_1_power_map_dimension) && ~isempty(obj.axis_1_power_map)
            io.writeAttribute(fid, [fullpath '/axis_1_power_map/dimension'], obj.axis_1_power_map_dimension);
        elseif ~isempty(obj.axis_1_power_map)
            error('Property `axis_1_power_map_dimension` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.axis_1_power_map_field_of_view) && ~isempty(obj.axis_1_power_map)
            io.writeAttribute(fid, [fullpath '/axis_1_power_map/field_of_view'], obj.axis_1_power_map_field_of_view);
        elseif ~isempty(obj.axis_1_power_map)
            error('Property `axis_1_power_map_field_of_view` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.axis_1_power_map)
            error('Property `axis_1_power_map_unit` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.axis_2_phase_map)
            ...
            error('Property `axis_2_phase_map` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.axis_2_phase_map_dimension) && ~isempty(obj.axis_2_phase_map)
            io.writeAttribute(fid, [fullpath '/axis_2_phase_map/dimension'], obj.axis_2_phase_map_dimension);
        elseif ~isempty(obj.axis_2_phase_map)
            error('Property `axis_2_phase_map_dimension` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.axis_2_phase_map_field_of_view) && ~isempty(obj.axis_2_phase_map)
            io.writeAttribute(fid, [fullpath '/axis_2_phase_map/field_of_view'], obj.axis_2_phase_map_field_of_view);
        elseif ~isempty(obj.axis_2_phase_map)
            error('Property `axis_2_phase_map_field_of_view` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.axis_2_phase_map)
            error('Property `axis_2_phase_map_unit` is required in `%s`.', fullpath);
        end
        ...
        if ~isempty(obj.axis_2_power_map_dimension) && ~isempty(obj.axis_2_power_map)
            io.writeAttribute(fid, [fullpath '/axis_2_power_map/dimension'], obj.axis_2_power_map_dimension);
        elseif ~isempty(obj.axis_2_power_map)
            error('Property `axis_2_power_map_dimension` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.axis_2_power_map_field_of_view) && ~isempty(obj.axis_2_power_map)
            io.writeAttribute(fid, [fullpath '/axis_2_power_map/field_of_view'], obj.axis_2_power_map_field_of_view);
        elseif ~isempty(obj.axis_2_power_map)
            error('Property `axis_2_power_map_field_of_view` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.axis_2_power_map)
            error('Property `axis_2_power_map_unit` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.focal_depth_image)
            error('Property `focal_depth_image_bits_per_pixel` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.focal_depth_image_dimension) && ~isempty(obj.focal_depth_image)
            io.writeAttribute(fid, [fullpath '/focal_depth_image/dimension'], obj.focal_depth_image_dimension);
        elseif ~isempty(obj.focal_depth_image)
            error('Property `focal_depth_image_dimension` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.focal_depth_image_field_of_view) && ~isempty(obj.focal_depth_image)
            io.writeAttribute(fid, [fullpath '/focal_depth_image/field_of_view'], obj.focal_depth_image_field_of_view);
        elseif ~isempty(obj.focal_depth_image)
            error('Property `focal_depth_image_field_of_view` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.focal_depth_image)
            error('Property `focal_depth_image_focal_depth` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.focal_depth_image)
            error('Property `focal_depth_image_format` is required in `%s`.', fullpath);
        end
        ...
        if ~isempty(obj.sign_map_dimension) && ~isempty(obj.sign_map)
            io.writeAttribute(fid, [fullpath '/sign_map/dimension'], obj.sign_map_dimension);
        elseif ~isempty(obj.sign_map)
            error('Property `sign_map_dimension` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.sign_map_field_of_view) && ~isempty(obj.sign_map)
            io.writeAttribute(fid, [fullpath '/sign_map/field_of_view'], obj.sign_map_field_of_view);
        elseif ~isempty(obj.sign_map)
            error('Property `sign_map_field_of_view` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.vasculature_image)
            ...
            error('Property `vasculature_image` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.vasculature_image)
            error('Property `vasculature_image_bits_per_pixel` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.vasculature_image_dimension) && ~isempty(obj.vasculature_image)
            io.writeAttribute(fid, [fullpath '/vasculature_image/dimension'], obj.vasculature_image_dimension);
        elseif ~isempty(obj.vasculature_image)
            error('Property `vasculature_image_dimension` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.vasculature_image_field_of_view) && ~isempty(obj.vasculature_image)
            io.writeAttribute(fid, [fullpath '/vasculature_image/field_of_view'], obj.vasculature_image_field_of_view);
        elseif ~isempty(obj.vasculature_image)
            error('Property `vasculature_image_field_of_view` is required in `%s`.', fullpath);
        end
        ...
        elseif ~isempty(obj.vasculature_image)
            error('Property `vasculature_image_format` is required in `%s`.', fullpath);
        end
    end
end
