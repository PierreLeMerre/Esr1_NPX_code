...
% PROPERTIES
properties
    clustering_interface; % Link to Clustering interface that was the source of the clustered data
...
methods
    ...
    function refs = export(obj, fid, fullpath, refs)
        ...
        if ~isempty(obj.clustering_interface)
            ...
            error('Property `clustering_interface` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.waveform_filtering)
            ...
            error('Property `waveform_filtering` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.waveform_mean)
            ...
            error('Property `waveform_mean` is required in `%s`.', fullpath);
        end
        if ~isempty(obj.waveform_sd)
            ...
            error('Property `waveform_sd` is required in `%s`.', fullpath);
        end
    end
end
