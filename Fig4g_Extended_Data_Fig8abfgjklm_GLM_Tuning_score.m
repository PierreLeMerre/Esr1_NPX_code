...
for f = f_start : f_stop
        ...
        load(['/Users/pierre/Documents/MATLAB/neuropixelPFC/Matlab/analysis/GLM/' nwb.general_session_id '.mat'])
...
load('/Users/pierre/Documents/MATLAB/Esr1_NPX_code/utilities/Colormaps/scatter_red_cmap.mat')
load('/Users/pierre/Documents/MATLAB/Esr1_NPX_code/utilities/Colormaps/scatter_blue_cmap.mat')
...
%% Scatter plot for WS NS distinction

    % opto vs Airpuff
    f6 = figure;
    hold on
    scatter(TS(2,RS==1),TS(4,RS==1),[],'MarkerEdgeColor',[0 0.5 0],...
                'MarkerFaceColor','none',...
                'LineWidth',2,'SizeData', 200);
    scatter(TS(2,FS==1),TS(4,FS==1),[],'MarkerEdgeColor',[1 0.5 0],...
                'MarkerFaceColor','none',...
...
%         'SizeData', 200);
    ...
    ylim([-20 20])
    %sgtitle(Genotype)
    set(gcf,'units','points','position',[1500,2000,500,550])
    
    %Save csv data
    % tuning scores
    id = 1 : size(TS(1,:),2);
    s = TS(1,:);
    s_rs = [TS(1,RS==1) nan(1,id(end)-size(TS(1,RS==1),2))];
    s_fs = [TS(1,FS==1) nan(1,id(end)-size(TS(1,FS==1),2))];
    o = TS(2,:);
    o_rs = [TS(2,RS==1) nan(1,id(end)-size(TS(2,RS==1),2))];
    o_fs = [TS(2,FS==1) nan(1,id(end)-size(TS(2,FS==1),2))];
    s2 = TS(3,:);
    s2_rs = [TS(3,RS==1) nan(1,id(end)-size(TS(3,RS==1),2))];
    s2_fs = [TS(3,FS==1) nan(1,id(end)-size(TS(3,FS==1),2))];
    a = TS(4,:);
    a_rs = [TS(4,RS==1) nan(1,id(end)-size(TS(4,RS==1),2))];
    a_fs = [TS(4,FS==1) nan(1,id(end)-size(TS(4,FS==1),2))];

    values = [id' s' s_rs' s_fs' o' o_rs' o_fs' s2' s2_rs' s2_fs' a' a_rs' a_fs'];
    Table = array2table(values);
    Table.Properties.VariableNames(1:13) = {'id', ...
        'tuning score Sound 1','tuning score Sound 1 WS','tuning score Sound 1 NS', ...
        'tuning score Opto','tuning score Opto WS','tuning score Opto NS', ...
        'tuning score Sound 2','tuning score Sound 2 WS','tuning score Sound 2 NS', ...
        'tuning score Air Puff','tuning score Air Puff WS','tuning score Air Puff NS'};
    writetable(Table,[dest '/tuning_scores_detail_' Genotype '.csv'],'Delimiter',',')    
